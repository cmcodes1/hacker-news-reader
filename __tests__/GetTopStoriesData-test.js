...
it('Testing getTopStoriesData API', async function () {
    ...
    const response = await topStoriesService.getTopStoriesData(30614987);

    expect(response).toStrictEqual({ "payload": { "by": "anonporridge", "id": 30618405, "parent": 30618127, "text": "Another reason I can think of is that as more and more legitimate traffic moves onto Tor internal (no exit nodes) it becomes increasingly non-viable for nation states to execute a blanket ban on Tor traffic as it would be too disruptive to people and the economy.", "time": 1646851411, "type": "comment" }, "status": "success" })
